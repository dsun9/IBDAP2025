...
class RIVGAE(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, rnn_layer_num, num_user, num_assertion, conv_type='GCN', bias=True):
        super(RIVGAE, self).__init__()
    ...
    def decode(self, z, B):
        user_nodes_mask = torch.zeros((self.num_user + self.num_assertion, self.output_dim), device=z.device)
        user_nodes_mask[:self.num_user, :] = 1.0
        asser_nodes_mask = torch.zeros((self.num_user + self.num_assertion, self.output_dim), device=z.device)
        asser_nodes_mask[self.num_user:, :] = 1.0
        ubm = torch.matmul(
            torch.matmul(z * user_nodes_mask, B),
            (z * asser_nodes_mask).t()
        )
        return torch.sigmoid(ubm + ubm.t())

    def forward(self, x_list, edge_idx_list, hx=None, B=None):
    ...
    def _reparameterized_sample(mean, std):
        ...
        return F.relu(sample.mul(std).add_(mean))
