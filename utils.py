...
def get_sp_adj_mat(file_path, full_node_list, sep='\t'):
    ...
    print('Loading graph from file: {}'.format(file_path))
    with open(file_path, 'r') as fp:
        ...
        for line in content_list[1:]:
            ...
            from_id = node2idx_dict[str(from_node)]
            to_id = node2idx_dict[str(to_node)]
...
def get_roc_scores(edges_pos, edges_neg, embs, B=None):
    emb = embs
    if B is not None:
        preds = torch.sigmoid((torch.matmul(emb[edges_pos[0], :], B) * emb[edges_pos[1], :]).sum(1))
        preds_neg = torch.sigmoid((torch.matmul(emb[edges_neg[0], :], B) * emb[edges_neg[1], :]).sum(1))
    else:
        preds = torch.sigmoid((emb[edges_pos[0], :] * emb[edges_pos[1], :]).sum(1))
        preds_neg = torch.sigmoid((emb[edges_neg[0], :] * emb[edges_neg[1], :]).sum(1))
    
    # preds_all = np.hstack([preds, preds_neg])
    # labels_all = np.hstack([np.ones(len(preds)), np.zeros(len(preds_neg))])
    preds_all = torch.cat([preds, preds_neg], dim=0)
    labels_all = torch.cat([torch.ones(len(preds)), torch.zeros(len(preds_neg))], dim=0).long().to(preds.device)
    
    f1 = BinaryF1Score().to(preds.device)
    auroc = BinaryAUROC().to(preds.device)
    ap = BinaryAveragePrecision().to(preds.device)
    return f1(preds_all, labels_all).item(), auroc(preds_all, labels_all).item(), ap(preds_all, labels_all).item()

    # return roc_auc_score(labels_all, preds_all), average_precision_score(labels_all, preds_all), f1_score(labels_all, preds_all)
...
def get_dynamic_gnn_methods():
    gnn_list = ['GCRN', 'EvolveGCN', 'VGRNN', 'RIVGAE']
    return dict(zip(gnn_list, np.ones(len(gnn_list), dtype=int)))
...
def get_supported_methods():
    method_list = ['DynAE', 'DynRNN', 'DynAERNN', 'GCRN', 'EvolveGCN', 'VGRNN', 'RIVGAE']
    return dict(zip(method_list, np.ones(len(method_list), dtype=int)))
