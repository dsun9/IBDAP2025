...
class DataLoader:
    ...
    def __init__(self, node_list, max_time_num, has_cuda=False):
        ...
        self.full_node_list = [str(node) for node in node_list]
    ...
    def get_neg_adj(self, origin_base_path, neg_edges_file, sep='\t', data_type='tensor'):
        graph_path = os.path.join(origin_base_path, neg_edges_file)
        spmat = get_sp_adj_mat(graph_path, self.full_node_list, sep=sep)
        if data_type == 'tensor':
            sptensor = sparse_mx_to_torch_sparse_tensor(spmat)
            return sptensor.cuda() if self.has_cuda else sptensor
        else:  # data_type == matrix
            return spmat
    ...
    def get_degree_feature_list(self, origin_base_path, start_idx, duration, sep='\t', init_type='gaussian', std=1e-4):
        ...
        for i, degrees in enumerate(degree_list):
            ...
            if init_type == 'gaussian':
                ...
                data = np.ones(degrees.shape[0], dtype=int)
    ...
    def get_node_label_list(self, nlabel_base_path, start_idx, duration, sep='\t'):
        nlabel_file_list_train = sorted(os.listdir(nlabel_base_path + '/train'))
        nlabel_file_list_test = sorted(os.listdir(nlabel_base_path + '/test'))
        node_label_list_train = []
        node_label_list_test = []
        label_dict = dict()
        for i in range(start_idx, min(start_idx + duration, self.max_time_num)):
            nlabel_file_path = os.path.join(nlabel_base_path, 'train', nlabel_file_list_train[i])
            df_nodes = pd.read_csv(nlabel_file_path, sep=sep, header=0, names=['node', 'label'])
            if len(df_nodes) == 0:
                node_label_list_train.append(torch.zeros((0, 2)).long().cuda() if self.has_cuda else torch.zeros((0, 2)).long())
                continue
            df_nodes['node'] = df_nodes['node'].apply(lambda x: self.node2idx_dict[str(x)])
            unique_labels = df_nodes['label'].unique()
            for label in unique_labels:
                label_dict[label] = 1
            node_labels = torch.from_numpy(df_nodes.values).long()
            node_label_list_train.append(node_labels.cuda() if self.has_cuda else node_labels)
        for i in range(start_idx, min(start_idx + duration, self.max_time_num)):
            nlabel_file_path = os.path.join(nlabel_base_path, 'test', nlabel_file_list_test[i])
            df_nodes = pd.read_csv(nlabel_file_path, sep=sep, header=0, names=['node', 'label'])
            if len(df_nodes) == 0:
                node_label_list_test.append(torch.zeros((0, 2)).long().cuda() if self.has_cuda else torch.zeros((0, 2)).long())
                continue
            df_nodes['node'] = df_nodes['node'].apply(lambda x: self.node2idx_dict[str(x)])
            unique_labels = df_nodes['label'].unique()
            for label in unique_labels:
                label_dict[label] = 1
            node_labels = torch.from_numpy(df_nodes.values).long()
            node_label_list_test.append(node_labels.cuda() if self.has_cuda else node_labels)
        return node_label_list_train, node_label_list_test, len(label_dict.keys())
